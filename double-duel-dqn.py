...
class QNetwork():
 ...
        with tf.variable_scope('conv2d.make'):
 ...
                                         stride=[1,1])
        ...
        self.outQ = self.value + tf.subtract(self.advantage, tf.reduce_mean(self.advantage,axis=1,keepdims=True))
...
def updateTargetGraph(tfVars, tau):
    ...
    opHolders = []
    for i,v in enumerate(tfVars[0:totalVars//2]):
        opHolders.append(
            tfVars[i+totalVars//2].assign(
                (v.value()*tau) +((1-tau)*tfVars[i+totalVars//2].value())
                )
            )
    return opHolders
    ...
    for i in range(numEpisodes):
        ...
        while j < maxEpisodes:
            ...
            if np.random.randn(1) < e or totalSteps < preTrainSteps:
                ...
                predict = ss.run(mainQN.predict, feed_dict={mainQN.scalarInput:[s]})
                a = predict[0]
            s1,r,d = env.step(a)
            s1 = processState(s1)
            totalSteps += 1
            ...
            if totalSteps > preTrainSteps:
                ...
                if totalSteps % updateFreq == 0:
                    ...
                    _ = ss.run(mainQN.updateModel,
                               feed_dict={mainQN.scalarInput:np.vstack(trainBatches[:,0]),
